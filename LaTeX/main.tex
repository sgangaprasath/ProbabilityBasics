...
In order to arrive at this general expression, let us assume a discrete sequence of random variables $X_1, X_2, ..., X_N$ that are sampled from i.i.d. Gaussian distribution with mean $0$ and variance $\sigma^2$. The joint probability density function for this sequence can be written as:
\begin{equation}P(X_1, X_2, ..., X_N) = \prod_{i=1}^N \frac{1}{\sqrt{2\pi\sigma^2}} \exp\left( -\frac{X_i^2}{2\sigma^2} \right)\end{equation}
This can be rewritten in matrix form as:
\begin{equation}P(X) = \frac{1}{(2\pi\sigma^2)^{N/2}} \exp\left( -\frac{1}{2\sigma^2} X^T X \right)\end{equation}
where $X$ is the column vector of random variables. In the limit as $N \to \infty$, this expression converges to the probability density functional for a Gaussian process. Let us consider a Gaussian process defined on the interval $[0, T]$ as a continuous limit of the discrete case. Assuming the process has mean function $\mu(t)$ and variance $\sigma^2$, we can express the probability density functional as:
\begin{equation}P[X] \propto \exp\left( -\frac{1}{2\sigma^2} \int_0^T dt (X(t) - \mu(t))^2 \right)\end{equation}
This expression captures the probability of observing a particular path $X(t)$ given the mean function $\mu(t)$ and variance $\sigma^2$.

If we assume that the stochastic process $X(t)$ is governed by a Langevin equation of the form:
\begin{equation}\frac{dX}{dt} = -\frac{\partial V(X)}{\partial X} + \eta(t)\end{equation}
where $V(X)$ is a potential function and $\eta(t)$ is a Gaussian white noise with mean zero, $\langle \eta(t) \rangle = 0$, and correlation function $\langle\eta(t) \eta(t')\rangle = 2D \delta(t - t')$, we can derive the corresponding action functional $S[X]$ for the path integral formulation. The action functional can be expressed as:
\begin{equation}S[X] = \int_0^T dt \left[ \frac{1}{4D} \left( \frac{dX}{dt} + \frac{\partial V(X)}{\partial X} \right)^2 \right]\end{equation}
In order to derive this expression, we start from the Langevin equation and consider the probability of a particular path $X(t)$ given the noise $\eta(t)$. The probability density for the noise can be written as:
\begin{equation}P[\eta] \propto \exp\left( -\frac{1}{4D} \int_0^T dt \, \eta(t)^2 \right)\end{equation}
Since $\eta(t) = \frac{dX}{dt} + \frac{\partial V(X)}{\partial X}$, we can substitute this expression into the probability density to obtain:
\begin{equation}P[X] \propto \exp\left( -\frac{1}{4D} \int_0^T dt \left( \frac{dX}{dt} + \frac{\partial V(X)}{\partial X} \right)^2 \right)\end{equation}
This leads us to the action functional $S[X]$ as defined above.

This action functional captures the dynamics of the stochastic process, where the first term represents the kinetic energy associated with the path, and the second term represents the potential energy landscape defined by $V(X)$. The path integral can then be used to compute various quantities of interest, such as the transition probability from an initial state $X(0) = x_0$ to a final state $X(T) = x_T$:
\begin{equation}P(x_T, T | x_0, 0) = \int_{X(0)=x_0}^{X(T)=x_T} \mathcal{D}[X] \, e^{-S[X]}\end{equation}
This expression allows us to compute the probability of transitioning between states in the presence of noise and potential landscapes, providing insights into the behavior of the system under stochastic dynamics. When $V(X) = 0$, the action functional simplifies to:
\begin{equation}S[X] = \int_0^T dt \left[ \frac{1}{4D} \left( \frac{dX}{dt} \right)^2 \right]\end{equation}
In this case, the path integral reduces to the well-known result for a free particle undergoing Brownian motion, where the transition probability is given by a Gaussian distribution:
\begin{equation}P(x_T, T | x_0, 0) = \frac{1}{\sqrt{4\pi D T}} \exp\left( -\frac{(x_T - x_0)^2}{4D T} \right)\end{equation}
This illustrates how the path integral formulation can be used to derive transition probabilities for stochastic processes, and how it can be applied to more complex systems by incorporating potential functions and interactions.

When we consider harmonic potentials of the form $V(X) = \frac{1}{2} k X^2$, the action functional becomes:
\begin{equation}S[X] = \int_0^T dt \left[ \frac{1}{4D} \left( \frac{dX}{dt} + kX \right)^2 \right]\end{equation}
In this case, the path integral can be evaluated to yield the transition probability for an Ornstein-Uhlenbeck process, which describes a particle undergoing Brownian motion in a harmonic potential. The transition probability for this process is given by:
\begin{equation}P(x_T, T | x_0, 0) = \sqrt{\frac{k}{2\pi D (1 - e^{-2kT})}} \exp\left( -\frac{k (x_T - x_0 e^{-kT})^2}{2D (1 - e^{-2kT})} \right)\end{equation}
This result demonstrates how the path integral formulation can be used to derive transition probabilities for systems with specific potential landscapes, and how it can capture the effects of both noise and deterministic forces on the dynamics of the system.
\end{singlespace}
\end{document}