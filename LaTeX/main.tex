...
\end{equation}
We know the standard Gaussian integral $\int_{0}^{\infty} e^{-v^2} dv = \frac{\sqrt{\pi}}{2}$. Substituting the Gaussian result back into our expression for $J$:
\begin{equation}
J = \frac{1}{\sqrt{a}} \left( \frac{\sqrt{\pi}}{2} \right) = \sqrt{\frac{\pi}{4a}}
\end{equation}
Now substitute $J$ back into the full expression for $I$ from Step 2 gives:
\begin{align}
% I &= 2e^{-2\sqrt{ab}} J \\
...
\end{equation}
Given the constant $\sqrt{\dfrac{\gamma}{2\pi}}$ and parameters $a = \dfrac{\gamma}{2}, b = -it$:
\begin{align}
...
\end{align}
Thus, for any $n$, $\phi_{Z_n}(t) = \phi_{Z_\infty}(t) = \exp\left( -\sqrt{-2i\gamma t} \right)$. To find the PDF $f_{Z_\infty}(z)$, we use the inversion formula:
\begin{equation}
...
To derive the probability distribution for correlated variables from scratch, we start with the simplest case (independent variables) and use a linear transformation to ``mix'' them. This is the most intuitive way to see where the covariance matrix $C$ and its inverse $C^{-1}$ actually come from.
...
\end{equation}
Here, the variables are uncorrelated. The ``covariance matrix'' is just the identity matrix $\mathbf{I}$. To create \textit{correlated} variables $X$, we define a linear transformation:
\begin{equation}
...
The \textbf{Onsager-Machlup functional} is essentially the ``Lagrangian'' for a stochastic process. Just as classical mechanics uses a Lagrangian to find the path of least action, stochastic mechanics uses this functional to find the most probable path a system will take.
...
\end{equation}
Because we changed variables from a function $\eta(t)$ to a function $x(t)$, we must include a functional Jacobian. In stochastic calculus, this term depends on the discretization (Itô vs. Stratonovich). For the standard Itô interpretation, the result is:
\begin{equation}
...
\end{equation}
This expression for the probability density functional can also be reinterpreted in terms of the Action functional $S[X]$:
...
\end{equation}
defines the \textit{most probable path} of the system. This path represents the trajectory that requires the ``least effort'' from the thermal noise to deviate the particle from its deterministic force field. The minimization of the action results in Euler-Lagrange equations that govern the dynamics of the most probable path and is given by:
\begin{equation}\frac{d}{dt} \left( \frac{\partial L}{\partial \dot{x}} \right) - \frac{\partial L}{\partial x} = 0.
\end{equation}
The Onsager-Machlup theory allows us to analyze the behavior of stochastic systems, compute transition probabilities, and understand the dynamics of rare events by studying the most probable paths that the system can take under the influence of noise.
% \begin{itemize}
%     \item \textbf{Most probable path:} If you want to know the most likely way a protein folds or a chemical reaction occurs, you minimize the integral of this functional (the ``Action'').
...
To derive the probability distribution for a particle in a harmonic potential $V(X) = \dfrac{1}{2}kX^2$ from scratch, we must solve the path integral for the Ornstein-Uhlenbeck process.

Because the Action is quadratic in $X$ and $\dot{X}$, the path integral is dominated by the classical path (the most probable path) that minimizes the action.
For a force $f(X) = -kX$, the Onsager-Machlup Lagrangian is:
\begin{equation}
L(X, \dot{X}) = \frac{(\dot{X} + kX)^2}{4D}
\end{equation}
(Note: We ignore the constant Jacobian term $\dfrac{k}{2}$ for now as it affects only the normalization $\mathcal{N}$). The Action $S[X]$ is:
\begin{equation}
...
To arrive at the Fokker-Planck equation from the path integral $P[X] \propto \exp(-S[X])$, we use the spato-temporal evolution of the probability density. The transition probability $P(x, t+\Delta t | x', t)$ acts as a propagator that moves the distribution forward by an infinitesimal time step $\Delta t$.
...
\begin{itemize}
    \item \textit{Term 1:} $\int G(\xi) P(x,t) d\xi = P(x,t)$
    \item \textit{Term 2:} $-\int \xi G(\xi) \frac{\partial P}{\partial x} d\xi = -f(x)\Delta t \dfrac{\partial P}{\partial x}$
    \item \textit{Term 3:} $\int \dfrac{\xi^2}{2} G(\xi) \dfrac{\partial^2 P}{\partial x^2} d\xi = D\Delta t \dfrac{\partial^2 P}{\partial x^2}$
\end{itemize}
